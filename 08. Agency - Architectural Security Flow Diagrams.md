# Agency – Architectural Security Encryption Flow Diagrams

This document provides detailed sequence diagrams for each major user flow in Agency, highlighting:
- 🔐 **Encrypted data** at each step
- 🚫 **Zero-knowledge boundaries** (where servers cannot decrypt)
- 🛡️ **Security mechanisms** and privacy guarantees

---

## Table of Contents

1. [Key Management Initialization](#1-key-management--initialization)
2. [Document Ingestion - Standalone Mode](#2-document-ingestion---standalone-mode)
3. [Document Ingestion - App-Managed Storage](#3-document-ingestion---app-managed-storage)
4. [Document Ingestion - BYOS Storage](#4-document-ingestion---byos-storage)
5. [Search RAG Chat - Standalone](#5-search--rag-chat---standalone)
6. [Search RAG Chat - Cloud-Connected](#6-search--rag-chat---cloud-connected)
7. [Document Download Decryption](#7-document-download--decryption)
8. [Share Link Creation Access](#8-share-link-creation--access)
9. [Team Creation Key Distribution](#9-team-creation--key-distribution)
10. [TEE Offload/Load Flow](#10-tee-offloadload-flow)

---

## Legend

```mermaid
flowchart LR
    subgraph "Trust Boundaries"
        T1[🟢 Trusted Zone<br/>User's Device]
        T2[🟡 Semi-Trusted<br/>TEE/Enclave]
        T3[🔴 Untrusted<br/>Cloud Storage]
    end
    
    subgraph "Data States"
        D1[📄 Plaintext Data]
        D2[🔐 Encrypted Data]
        D3[🔑 Encryption Keys]
        D4[📊 Metadata Only]
    end
```

> **Zero-Knowledge Principle:** In all flows, the cloud infrastructure (Go API, Supabase, S3 storage) only sees:
> - 🔐 Encrypted blobs
> - 🔐 Encrypted filenames (hex-encoded)
> - 📊 Non-sensitive metadata (IDs, timestamps, sizes)
> - ❌ **Never** plaintext content, filenames, or decryption keys

---

## 1. Key Management Initialization

### User Master Key Generation

```mermaid
sequenceDiagram
    participant User
    participant TauriApp as 🟢 Tauri App
    participant PyBackend as 🟢 Python Backend
    participant LocalVault as 🟢 Encrypted Vault

    Note over User,LocalVault: 🟢 ALL IN TRUSTED ZONE (User's Device)
    
    User->>TauriApp: First launch
    TauriApp->>PyBackend: Initialize crypto system
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Key Generation (Local Only)
        PyBackend->>PyBackend: Generate 🔑 User Master Key<br/>(AES-256 / XChaCha20-Poly1305)
        PyBackend->>PyBackend: Derive salt + KDF params
        PyBackend->>LocalVault: Store 🔐 Encrypted Master Key<br/>(protected by user password/biometric)
    end
    
    PyBackend-->>TauriApp: ✅ Crypto initialized
    TauriApp-->>User: Ready to use
    
    Note over LocalVault: 🔑 User Master Key NEVER leaves device
    Note over LocalVault: Used to encrypt/decrypt org team keys
```

**Security Properties:**
- 🔑 User Master Key generated locally using secure random number generator
- 🔐 Protected by user password or biometric authentication
- 🚫 **Zero-Knowledge Boundary:** Master key NEVER transmitted to cloud
- 💾 Stored in encrypted local vault (filesystem encryption)

---

## 2. Document Ingestion - Standalone Mode

```mermaid
sequenceDiagram
    participant User
    participant TauriApp as 🟢 Tauri App
    participant PyBackend as 🟢 Python Backend
    participant Postgres as 🟢 Local Postgres
    participant Qdrant as 🟢 Local Qdrant
    participant Neo4j as 🟢 Local Neo4j
    participant Vault as 🟢 Encrypted Vault

    Note over User,Vault: 🟢 100% LOCAL - FULLY OFFLINE CAPABLE
    
    User->>TauriApp: Drag drop 📄 document.pdf
    TauriApp->>PyBackend: POST /ingest (file path)
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Document Processing Pipeline
        
        PyBackend->>PyBackend: 1️⃣ Parse document<br/>(extract text, tables, metadata)
        Note over PyBackend: 📄 Plaintext exists ONLY in memory
        
        PyBackend->>PyBackend: 2️⃣ Chunk text by title strategy
        
        PyBackend->>PyBackend: 3️⃣ Generate embeddings<br/>(local LLM model)
        
        PyBackend->>PyBackend: 4️⃣ Extract entities relationships<br/>(NER for people, orgs, events)
        
        PyBackend->>PyBackend: 5️⃣ Get 🔑 Team Key from memory
        PyBackend->>PyBackend: 6️⃣ Encrypt original file<br/>🔐 AES-256-CBC(Team Key, file)
        PyBackend->>PyBackend: 7️⃣ Encrypt filename<br/>🔐 hex(encrypt(Team Key, "document.pdf"))
    end
    
    rect rgb(240, 242, 245)
        Note over PyBackend,Vault: Persistence Layer
        
        PyBackend->>Postgres: Store document metadata<br/>📊 {id, 🔐encrypted_filename, size, created_at}
        PyBackend->>Postgres: Store 📄 plaintext chunks<br/>(searchable, team-scoped)
        
        PyBackend->>Qdrant: Store vector embeddings<br/>(collection per team)
        
        PyBackend->>Neo4j: Store entities relations<br/>(graph nodes, team-scoped)
        
        PyBackend->>Vault: Store 🔐 encrypted file blob<br/>(original document encrypted)
    end
    
    PyBackend-->>TauriApp: ✅ Ingestion complete
    TauriApp-->>User: Document added to knowledge base
    
    Note over Postgres,Vault: 📄 Plaintext chunks in local DBs for search<br/>🔐 Original file encrypted in vault<br/>🚫 NO cloud transmission in standalone mode
```

**Security Properties:**
- 📄 Plaintext exists ONLY in memory during processing
- 🔐 Original file encrypted with team key before storage
- 📄 Text chunks stored in plaintext locally for search (team-scoped)
- 🟢 All data stays on user's device
- 🚫 **Zero-Knowledge Boundary:** Not applicable (no cloud in standalone mode)
- 💾 Encrypted vault protects original files at rest

**Privacy Guarantees:**
- ✅ Fully offline capable - no network calls required
- ✅ Local search and embeddings - no data leaves device
- ✅ Multi-team isolation via local database scoping

---

## 3. Document Ingestion - App-Managed Storage

```mermaid
sequenceDiagram
    participant User
    participant TauriApp as 🟢 Tauri App
    participant PyBackend as 🟢 Python Backend
    participant LocalDBs as 🟢 Local DBs<br/>(Postgres/Qdrant/Neo4j)
    participant GoAPI as 🔴 Go API<br/>(api.getagency.app)
    participant Supabase as 🔴 Supabase
    participant S3 as 🔴 App-Managed S3<br/>(Wasabi/Backblaze)

    Note over User,LocalDBs: 🟢 TRUSTED ZONE
    Note over GoAPI,S3: 🔴 UNTRUSTED ZONE (Zero-Knowledge Boundary)
    
    User->>TauriApp: Upload document.pdf
    TauriApp->>PyBackend: POST /ingest
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Local Processing (Same as Standalone)
        PyBackend->>PyBackend: 1️⃣ Parse, chunk, embed, extract entities
        PyBackend->>LocalDBs: 2️⃣ Store chunks, vectors, graph locally
        PyBackend->>PyBackend: 3️⃣ Get 🔑 Team Key from session
    end
    
    rect rgb(245, 242, 240)
        Note over PyBackend,S3: 🚫 ZERO-KNOWLEDGE UPLOAD
        
        PyBackend->>PyBackend: 4️⃣ Encrypt file<br/>🔐 ciphertext = AES-256-CBC(🔑Team Key, file)
        PyBackend->>PyBackend: 5️⃣ Encrypt filename<br/>🔐 enc_name = hex(encrypt(🔑Team Key, "document.pdf"))
        
        PyBackend->>GoAPI: POST /upload/initiate<br/>📊 {team_id, 🔐enc_filename, size, mime_type}
        Note over GoAPI: Sees ONLY encrypted filename
        
        GoAPI->>Supabase: Check permissions quota
        Supabase-->>GoAPI: ✅ Authorized
        
        GoAPI->>GoAPI: Generate presigned S3 URL<br/>(App-Managed bucket)
        GoAPI-->>PyBackend: {presigned_url, file_id}
        
        PyBackend->>S3: PUT 🔐 encrypted_blob<br/>(directly to S3)
        Note over S3: Stores ONLY ciphertext
        S3-->>PyBackend: ✅ Upload complete
        
        PyBackend->>GoAPI: POST /upload/complete<br/>📊 {file_id, checksum}
        GoAPI->>Supabase: Record file metadata<br/>📊 {id, team_id, 🔐enc_filename, size, created_at}
    end
    
    PyBackend-->>TauriApp: ✅ Sync complete
    TauriApp-->>User: Document synced to cloud
    
    Note over S3: 🔐 Only encrypted blob stored<br/>🔐 Filename is encrypted hex-encoded<br/>🚫 Cloud CANNOT decrypt without Team Key
    Note over GoAPI,Supabase: 🔴 ZERO-KNOWLEDGE: No plaintext access<br/>🔑 Team Key never transmitted
```

**Security Properties:**
- 🔐 File encrypted with team key BEFORE leaving device
- 🔐 Filename encrypted and hex-encoded before transmission
- 🔑 Team key NEVER sent to cloud (stays in local memory)
- 📊 Cloud sees only: encrypted blob, encrypted filename, metadata (size, timestamps)
- 🚫 **Zero-Knowledge Boundary:** Go API, Supabase, and S3 cannot decrypt content

**Data Flow:**
1. 🟢 **Trusted:** Processing and encryption on local device
2. 🔴 **Untrusted:** Encrypted upload to cloud storage
3. 🔴 **Untrusted:** Cloud stores ciphertext only

---

## 4. Document Ingestion - BYOS Storage

```mermaid
sequenceDiagram
    participant User
    participant TauriApp as 🟢 Tauri App
    participant PyBackend as 🟢 Python Backend
    participant LocalDBs as 🟢 Local DBs
    participant GoAPI as 🔴 Go API
    participant Supabase as 🔴 Supabase
    participant UserS3 as 🔴 User's S3 Bucket<br/>(AWS/GCP/Azure/R2)

    Note over User,LocalDBs: 🟢 TRUSTED ZONE
    Note over GoAPI,UserS3: 🔴 UNTRUSTED ZONE (User-Controlled Storage)
    
    User->>TauriApp: Upload document.pdf
    TauriApp->>PyBackend: POST /ingest
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Local Processing
        PyBackend->>PyBackend: 1️⃣ Parse, chunk, embed, NER
        PyBackend->>LocalDBs: 2️⃣ Store locally
        PyBackend->>PyBackend: 3️⃣ Get 🔑 Team Key
    end
    
    rect rgb(245, 242, 240)
        Note over PyBackend,UserS3: BYOS Upload Flow
        
        PyBackend->>PyBackend: 4️⃣ Encrypt file + filename<br/>🔐 AES-256-CBC(🔑Team Key, data)
        
        PyBackend->>GoAPI: GET /storage/byos/credentials<br/>📊 {org_id, team_id}
        
        GoAPI->>Supabase: Fetch 🔐 encrypted BYOS credentials
        Note over Supabase: Credentials encrypted with 🔑 Org Key
        
        GoAPI->>GoAPI: Get 🔐 encrypted credentials<br/>(still encrypted)
        GoAPI-->>PyBackend: {🔐encrypted_credentials, provider, bucket}
        
        PyBackend->>PyBackend: Decrypt credentials locally<br/>🔑 Org Key (from session)
        Note over PyBackend: 🔑 Plaintext credentials ONLY in local memory
        
        PyBackend->>PyBackend: Initialize provider SDK<br/>(AWS/Azure/GCP/S3-compatible)
        
        PyBackend->>UserS3: PUT 🔐 encrypted_blob<br/>(using user's bucket)
        Note over UserS3: User owns controls storage
        UserS3-->>PyBackend: ✅ Upload complete
        
        PyBackend->>GoAPI: POST /files/register<br/>📊 {team_id, 🔐enc_filename, size, storage_path}
        GoAPI->>Supabase: Record file metadata
    end
    
    PyBackend-->>TauriApp: ✅ Synced to BYOS
    TauriApp-->>User: Document in your bucket
    
    Note over UserS3: 🔐 User controls storage retention<br/>🔐 Only encrypted data stored<br/>🔑 Agency never stores plaintext credentials
    Note over GoAPI: 🔴 Cloud stores encrypted credentials<br/>🔑 Decryption happens locally only
```

**Security Properties:**
- 🔐 File and filename encrypted before upload
- 🔑 BYOS credentials encrypted with org key in cloud
- 🔑 Credentials decrypted ONLY in local backend (never in cloud)
- 🏢 User owns and controls the storage bucket
- 🚫 **Zero-Knowledge Boundary:** Cloud API never sees plaintext credentials or file content
- 🔐 Provider SDK initialized locally with decrypted credentials

**BYOS Advantages:**
- ✅ User controls data residency and retention policies
- ✅ Direct control over storage costs and lifecycle
- ✅ Credentials protected via org key encryption
- ✅ Agency cloud never handles plaintext storage credentials

---

## 5. Search RAG Chat - Standalone

```mermaid
sequenceDiagram
    participant User
    participant TauriApp as 🟢 Tauri App
    participant PyBackend as 🟢 Python Backend
    participant Postgres as 🟢 Postgres
    participant Qdrant as 🟢 Qdrant
    participant Neo4j as 🟢 Neo4j
    participant LLM as 🟢 Local LLM

    Note over User,LLM: 🟢 100% LOCAL - FULLY PRIVATE
    
    User->>TauriApp: Search: "Q3 budget overruns"
    TauriApp->>PyBackend: POST /search<br/>📊 {query, team_id, filters}
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Hybrid Search Pipeline
        
        PyBackend->>PyBackend: 1️⃣ Generate query embedding<br/>(local model)
        
        par Parallel Search
            PyBackend->>Postgres: Keyword metadata search<br/>📊 (team-scoped)
            Postgres-->>PyBackend: Matching 📄 chunks
        and
            PyBackend->>Qdrant: Vector similarity search<br/>📊 (team collection)
            Qdrant-->>PyBackend: Similar 📄 chunks
        and
            PyBackend->>Neo4j: Graph query for related entities<br/>📊 (team graph)
            Neo4j-->>PyBackend: Related entities
        end
        
        PyBackend->>PyBackend: 2️⃣ Merge rank results
        PyBackend->>PyBackend: 3️⃣ Group by document
    end
    
    PyBackend-->>TauriApp: 📄 Search results with citations
    TauriApp-->>User: Shows documents relevant passages
    
    User->>TauriApp: Chat: "Summarize the budget issues"
    TauriApp->>PyBackend: POST /chat<br/>📊 {message, team_id, context}
    
    rect rgb(243, 245, 243)
        Note over PyBackend,LLM: RAG Chat Pipeline
        
        PyBackend->>PyBackend: 1️⃣ Retrieve relevant chunks<br/>(same hybrid search)
        
        PyBackend->>PyBackend: 2️⃣ Build RAG prompt<br/>📄 Context: {chunks}<br/>📄 Question: {message}
        
        PyBackend->>LLM: Generate response<br/>(local LLM in memory)
        Note over LLM: Model downloaded managed by app
        
        LLM-->>PyBackend: 📄 Generated answer + sources
        
        PyBackend->>Postgres: Save to 📊 query_history<br/>(team-scoped)
    end
    
    PyBackend-->>TauriApp: 📄 Answer with document citations
    TauriApp-->>User: Shows AI response with sources
    
    Note over PyBackend,LLM: 🚫 Zero external API calls<br/>🔑 All data stays on device<br/>🟢 Works fully offline
```

**Security Properties:**
- 🟢 All processing happens locally on user's device
- 📄 Search operates on plaintext chunks (team-scoped in local DB)
- 🤖 Local LLM model (no external AI API calls)
- 🚫 **Zero-Knowledge Boundary:** Not applicable (fully local)
- 🔒 Team isolation via database scoping

**Privacy Guarantees:**
- ✅ No network calls - fully offline capable
- ✅ No telemetry or analytics sent to cloud
- ✅ Query history stored locally only
- ✅ Complete data privacy

---

## 6. Search RAG Chat - Cloud-Connected

```mermaid
sequenceDiagram
    participant User
    participant TauriApp as 🟢 Tauri App
    participant PyBackend as 🟢 Python Backend
    participant LocalDBs as 🟢 Local DBs<br/>(Postgres/Qdrant/Neo4j)
    participant GoAPI as 🔴 Go API
    participant Supabase as 🔴 Supabase

    Note over User,LocalDBs: 🟢 TRUSTED ZONE (All Content Processing)
    Note over GoAPI,Supabase: 🔴 UNTRUSTED ZONE (Metadata Only)
    
    User->>TauriApp: Search Chat query
    TauriApp->>PyBackend: POST /search or /chat
    
    rect rgb(245, 245, 245)
        Note over PyBackend,LocalDBs: Search RAG (100% Local)
        
        PyBackend->>LocalDBs: Hybrid search (keyword + vector + graph)
        LocalDBs-->>PyBackend: 📄 Matching chunks
        
        PyBackend->>PyBackend: Generate embeddings RAG response<br/>(local LLM)
        
        PyBackend->>LocalDBs: Save 📊 query_history locally
    end
    
    opt Cloud Sync (Optional Background)
        PyBackend->>GoAPI: POST /analytics/query-log<br/>📊 {team_id, query_hash, timestamp}
        Note over GoAPI: Receives ONLY anonymized metadata<br/>❌ NO query text, results, or content
        
        GoAPI->>Supabase: Store usage stats<br/>📊 {query_count, team_id, date}
        Note over Supabase: For billing org analytics only
    end
    
    PyBackend-->>TauriApp: 📄 Search results / chat response
    TauriApp-->>User: Display results
    
    Note over LocalDBs: 🟢 ALL content processing local<br/>📄 Plaintext chunks never leave device<br/>🤖 LLM runs locally
    Note over GoAPI,Supabase: 🔴 Cloud sees ONLY:<br/>📊 Usage counts timestamps<br/>❌ NO query content or results
```

**Security Properties:**
- 🟢 Search and RAG processing identical to standalone (100% local)
- 📊 Optional usage telemetry (anonymized, no content)
- 🔑 Query text and results NEVER sent to cloud
- 🚫 **Zero-Knowledge Boundary:** Cloud cannot see queries or search results
- 📈 Usage stats for billing purposes only

**What Cloud Sees:**
- ✅ Number of queries per team
- ✅ Timestamps and usage patterns
- ❌ **Never:** Query text, search results, document content

---

## 7. Document Download Decryption

```mermaid
sequenceDiagram
    participant User
    participant TauriApp as 🟢 Tauri App
    participant PyBackend as 🟢 Python Backend
    participant LocalCache as 🟢 Local Cache
    participant GoAPI as 🔴 Go API
    participant Supabase as 🔴 Supabase
    participant S3 as 🔴 Cloud Storage<br/>(App-Managed or BYOS)

    Note over User,LocalCache: 🟢 TRUSTED ZONE
    Note over GoAPI,S3: 🔴 UNTRUSTED ZONE
    
    User->>TauriApp: Open document "report.pdf"
    TauriApp->>PyBackend: GET /documents/{id}/content
    
    PyBackend->>LocalCache: Check local vault
    
    alt Document cached locally
        LocalCache-->>PyBackend: 🔐 Encrypted blob (cached)
    else Need to download
        rect rgb(245, 242, 240)
            Note over PyBackend,S3: Authenticated Download
            
            PyBackend->>GoAPI: GET /files/{file_id}/download-url<br/>📊 {team_id, file_id}
            
            GoAPI->>Supabase: Verify permissions<br/>📊 Check team membership
            Supabase-->>GoAPI: ✅ User authorized
            
            GoAPI->>GoAPI: Generate time-limited presigned URL<br/>(or CF Worker auth token)
            GoAPI-->>PyBackend: {download_url, 🔐enc_filename}
            
            PyBackend->>S3: GET 🔐 encrypted_blob<br/>(using presigned URL)
            Note over S3: Downloads ciphertext
            S3-->>PyBackend: 🔐 Encrypted file
            
            PyBackend->>LocalCache: Cache 🔐 encrypted blob locally
        end
    end
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Client-Side Decryption
        
        PyBackend->>PyBackend: Get 🔑 Team Key from session
        PyBackend->>PyBackend: Decrypt file<br/>📄 plaintext = AES-256-CBC-decrypt(🔑Team Key, 🔐blob)
        PyBackend->>PyBackend: Decrypt filename<br/>📄 "report.pdf" = decrypt(🔑Team Key, 🔐enc_name)
        
        Note over PyBackend: 📄 Plaintext exists ONLY in memory
    end
    
    PyBackend-->>TauriApp: 📄 Plaintext file content
    TauriApp-->>User: Display document
    
    Note over S3: 🔐 Only ciphertext transmitted<br/>🚫 Cloud never sees plaintext
    Note over PyBackend: 🔑 Decryption happens locally<br/>🔑 Team Key never sent to cloud
```

**Security Properties:**
- 🔐 Cloud storage serves only encrypted blobs
- 🔑 Decryption happens exclusively in local backend
- 🔑 Team key stays in local memory (never transmitted)
- 🔒 Presigned URLs are time-limited and permission-checked
- 🚫 **Zero-Knowledge Boundary:** Cloud cannot decrypt files

**Download Flow:**
1. 🔴 **Untrusted:** Download encrypted blob from cloud
2. 🟢 **Trusted:** Decrypt locally with team key
3. 📄 **Plaintext:** Exists only in local memory/cache

---

## 8. Share Link Creation Access

```mermaid
sequenceDiagram
    participant Owner as 🟢 Document Owner
    participant PyBackend as 🟢 Python Backend
    participant GoAPI as 🔴 Go API
    participant Supabase as 🔴 Supabase
    participant ShareWorker as 🔴 CF Worker<br/>(shares.getagency.app)
    participant Recipient as 🔵 Share Recipient
    participant RecipientApp as 🟢 Recipient's App

    Note over Owner,PyBackend: 🟢 OWNER'S DEVICE
    Note over GoAPI,ShareWorker: 🔴 UNTRUSTED CLOUD
    Note over Recipient,RecipientApp: 🟢 RECIPIENT'S DEVICE
    
    Owner->>PyBackend: Create share link for document
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Share Link Generation
        
        PyBackend->>PyBackend: Get 🔑 Team Key
        PyBackend->>PyBackend: Generate random 🔑 Share Key
        PyBackend->>PyBackend: Encrypt document<br/>🔐 share_cipher = encrypt(🔑Share Key, file)
        PyBackend->>PyBackend: Optional: encrypt with password<br/>🔐 password_wrapped_key
    end
    
    PyBackend->>GoAPI: POST /shares/create<br/>📊 {file_id, 🔐share_cipher_ref, expiry, password_hash}
    
    GoAPI->>Supabase: Store share metadata<br/>📊 {share_id, file_id, expiry, password_hash, access_count}
    Note over Supabase: NO encryption keys stored<br/>Only metadata password hash
    
    GoAPI-->>PyBackend: {share_url: "shares.getagency.app/{share_id}"}
    PyBackend->>PyBackend: Append 🔑 Share Key to URL fragment<br/>(shares.getagency.app/{id}#key={share_key})
    
    PyBackend-->>Owner: 🔗 Share URL with key in fragment
    Owner->>Recipient: Send share link
    
    rect rgb(240, 242, 245)
        Note over Recipient,RecipientApp: Share Link Access
        
        Recipient->>ShareWorker: Open shares.getagency.app/{id}#key=...
        Note over ShareWorker: Fragment (#key=...) NOT sent to server<br/>(stays in browser)
        
        ShareWorker->>Supabase: GET /shares/{id}/metadata
        Supabase-->>ShareWorker: 📊 {expiry, password_required, file_ref}
        
        alt Password protected
            ShareWorker->>Recipient: Prompt for password
            Recipient->>ShareWorker: Enter password
            ShareWorker->>Supabase: Verify password hash
        end
        
        ShareWorker->>Supabase: Log access<br/>📊 {share_id, timestamp, IP_hash}
        
        alt App-Managed Storage
            ShareWorker-->>Recipient: HTML page with download button
            Note over Recipient: 🔑 Share Key from URL fragment (in browser memory only)
            
            Recipient->>ShareWorker: Click download
            ShareWorker->>GoAPI: Generate presigned URL for 🔐 share_cipher
            GoAPI-->>ShareWorker: {download_url}
            
            Recipient->>ShareWorker: Download 🔐 encrypted blob
            
            Note over Recipient: Client-side decryption in browser
            Recipient->>Recipient: JavaScript: decrypt(🔑Share Key, 🔐blob)
            Recipient->>Recipient: Download 📄 plaintext file
            
        else BYOS Storage
            ShareWorker-->>Recipient: Deep link to app<br/>agency://open-share/{id}#key=...
            Recipient->>RecipientApp: Open in Agency app (if registered user)
            RecipientApp->>RecipientApp: Decrypt with 🔑 Share Key
        end
    end
    
    Note over ShareWorker: 🔴 Cloud sees:<br/>📊 Access logs, expiry, password hash<br/>❌ NO encryption keys (in fragment)<br/>❌ NO plaintext content
    Note over Recipient: 🔑 Share Key stays in browser/app<br/>🔐 Decryption happens client-side
```

**Security Properties:**
- 🔑 Share key transmitted in URL fragment (never sent to server)
- 🔐 Document encrypted specifically for sharing (separate from team key)
- 🔒 Optional password protection (hashed, not stored plaintext)
- 📊 Access logs for audit trail
- 🚫 **Zero-Knowledge Boundary:** Cloud never sees share key or plaintext
- ⏰ Time-limited share links with expiry

**Share Link Security:**
- ✅ Share key in URL fragment (client-side only)
- ✅ Password protection available
- ✅ Access logging and revocation
- ✅ Client-side decryption (browser or app)
- ❌ Cloud cannot decrypt shared content

**BYOS vs App-Managed:**
- **App-Managed:** Browser decryption via JavaScript
- **BYOS:** Deep link to app (registered users only)

---

## 9. Team Creation Key Distribution

```mermaid
sequenceDiagram
    participant Admin as 🟢 Org Admin
    participant PyBackend as 🟢 Python Backend
    participant GoAPI as 🔴 Go API
    participant Supabase as 🔴 Supabase
    participant Member as 🟢 Team Member

    Note over Admin,PyBackend: 🟢 ADMIN'S DEVICE
    Note over GoAPI,Supabase: 🔴 UNTRUSTED CLOUD
    Note over Member: 🟢 MEMBER'S DEVICE
    
    Admin->>PyBackend: Create new team "Marketing"
    
    rect rgb(245, 245, 245)
        Note over PyBackend: Team Key Generation
        
        PyBackend->>PyBackend: Generate 🔑 Team Key<br/>(AES-256 random)
        PyBackend->>PyBackend: Get 🔑 User Master Key from vault
        PyBackend->>PyBackend: Encrypt for admin<br/>🔐 encrypted_team_key = encrypt(🔑User Master Key, 🔑Team Key)
    end
    
    PyBackend->>GoAPI: POST /teams/create<br/>📊 {org_id, name, 🔐encrypted_team_key_for_admin}
    
    GoAPI->>Supabase: Create team record<br/>📊 {id, org_id, name, created_by}
    GoAPI->>Supabase: Store 🔐 encrypted_team_key in user_team_keys<br/>📊 {user_id: admin, team_id, 🔐wrapped_key}
    
    Note over Supabase: Stores 🔐 encrypted team key<br/>🔑 Cannot decrypt without user's master key
    
    GoAPI-->>PyBackend: {team_id}
    PyBackend-->>Admin: ✅ Team created
    
    Admin->>PyBackend: Invite member to team
    PyBackend->>GoAPI: POST /teams/{id}/invite<br/>📊 {email, role}
    
    GoAPI->>Supabase: Create invitation<br/>📊 {team_id, email, role, invited_by}
    GoAPI->>Supabase: Send invite email
    
    rect rgb(240, 242, 245)
        Note over Member: Member Accepts Invitation
        
        Member->>GoAPI: Accept invite (via email link)
        GoAPI->>Supabase: Mark invitation accepted
        GoAPI->>Supabase: Add to team_members<br/>📊 {team_id, user_id, role}
        
        Note over Member,PyBackend: Key Distribution
        
        Member->>PyBackend: Login sync teams
        PyBackend->>GoAPI: GET /teams/mine
        GoAPI->>Supabase: Get user's teams
        Supabase-->>GoAPI: 📊 Teams list (no keys yet)
        GoAPI-->>PyBackend: 📊 {teams: [{id, name}]}
        
        PyBackend->>GoAPI: Request team key for "Marketing"
        Note over GoAPI: Determine key distribution method
        
        alt Org in Compliance Mode
            Note over GoAPI: Org admins auto-added to teams
            GoAPI->>Supabase: Get admin's 🔐 encrypted_team_key
            GoAPI-->>PyBackend: 🔐 encrypted_key (for admin)
            
            PyBackend->>Admin: Request to share key with member
            Admin->>PyBackend: Approve
            
            PyBackend->>PyBackend: Decrypt with admin's 🔑 Master Key<br/>🔑 Team Key = decrypt(🔑Admin Master, 🔐encrypted_key)
            PyBackend->>PyBackend: Re-encrypt for member<br/>🔐 member_wrapped = encrypt(🔑Member Master Key, 🔑Team Key)
            
            PyBackend->>GoAPI: POST /teams/{id}/keys/grant<br/>📊 {user_id: member, 🔐member_wrapped_key}
            GoAPI->>Supabase: Store 🔐 encrypted_team_key for member
            
        else Org in Privacy Mode
            Note over GoAPI: Manual key sharing required
            Admin->>PyBackend: Share team key with member
            PyBackend->>PyBackend: Same re-encryption flow
        end
    end
    
    Member->>PyBackend: Fetch my team keys
    PyBackend->>GoAPI: GET /teams/{id}/keys/mine
    GoAPI->>Supabase: Get 🔐 encrypted_team_key for user
    Supabase-->>GoAPI: 🔐 user's wrapped key
    GoAPI-->>PyBackend: 🔐 encrypted_team_key
    
    PyBackend->>PyBackend: Decrypt with 🔑 User Master Key<br/>🔑 Team Key = decrypt(🔑User Master, 🔐wrapped)
    PyBackend->>PyBackend: Hold 🔑 Team Key in memory session
    
    PyBackend-->>Member: ✅ Team access granted
    
    Note over Supabase: 🔴 Stores ONLY 🔐 encrypted team keys<br/>🔑 Cannot decrypt without user master keys<br/>🔑 Each user has their own wrapped copy
    Note over PyBackend: 🟢 Decryption happens locally<br/>🔑 Team key in memory only
```

**Security Properties:**
- 🔑 Team key generated locally by admin
- 🔐 Team key encrypted separately for each team member (wrapped with their master key)
- 🔑 Cloud stores only encrypted team keys (per-user wrapped)
- 🚫 **Zero-Knowledge Boundary:** Cloud cannot decrypt team keys
- 🔒 Privacy Mode: Manual key sharing required
- 🔒 Compliance Mode: Org admins auto-granted access

**Key Distribution Model:**
- ✅ Each user gets their own encrypted copy of team key
- ✅ Wrapped with user's master key (unique per user)
- ✅ Cloud cannot unwrap or access team keys
- ✅ Supports key rotation and revocation

**Privacy vs Compliance Mode:**
- **Privacy Mode:** Org admins not auto-added to teams, manual key sharing
- **Compliance Mode:** Org admins granted access to all team keys for audit purposes

---

## 10. TEE Offload/Load Flow

```mermaid
sequenceDiagram
    participant User
    participant PyBackend as 🟢 Python Backend
    participant LocalDBs as 🟢 Local DBs
    participant GoAPI as 🔴 Go API
    participant Supabase as 🔴 Supabase
    participant TEE as 🟡 TEE Instance<br/>(Confidential VM)
    participant UserCloud as 🔴 User's Cloud<br/>(AWS/Azure/GCP)

    Note over User,LocalDBs: 🟢 TRUSTED ZONE
    Note over GoAPI,UserCloud: 🔴 UNTRUSTED INFRASTRUCTURE
    Note over TEE: 🟡 SEMI-TRUSTED (Hardware-Protected Enclave)
    
    rect rgb(245, 242, 240)
        Note over User,PyBackend: Threshold Exceeded (500GB+)
        
        PyBackend->>PyBackend: Detect team dataset \u003e 500GB
        PyBackend->>User: ⚠️ TEE required for team size
        User->>PyBackend: Enable TEE for team
    end
    
    rect rgb(240, 242, 245)
        Note over PyBackend,TEE: TEE Provisioning
        
        PyBackend->>GoAPI: POST /tee/provision<br/>📊 {team_id, cloud_provider: "AWS", size: "large"}
        
        GoAPI->>Supabase: Get org's BYO cloud credentials<br/>🔐 encrypted creds
        GoAPI->>Supabase: Reserve TEE instance metadata
        
        GoAPI->>UserCloud: Provision confidential VM<br/>(using org's cloud account)
        Note over UserCloud: Org pays cloud costs directly<br/>VM in org's account
        
        UserCloud-->>GoAPI: 📊 {instance_id, attestation_cert}
        
        GoAPI->>GoAPI: Verify TEE attestation<br/>(ensure genuine secure enclave)
        GoAPI->>Supabase: Store TEE metadata<br/>📊 {team_id, instance_id, status: "ready"}
        
        GoAPI-->>PyBackend: ✅ TEE provisioned
    end
    
    rect rgb(242, 242, 240)
        Note over PyBackend,TEE: Secure Data Offload
        
        PyBackend->>PyBackend: Get 🔑 Team Key from vault
        PyBackend->>LocalDBs: Export Qdrant vectors Neo4j graph
        LocalDBs-->>PyBackend: 📄 Plaintext index data
        
        PyBackend->>PyBackend: Encrypt for TEE<br/>🔐 tee_blob = encrypt(🔑TEE Session Key, data)
        Note over PyBackend: TEE Session Key derived from Team Key<br/>+ TEE attestation
        
        PyBackend->>TEE: POST /tee/load<br/>🔐 encrypted_index_blob
        Note over TEE: TLS + attestation-based auth
        
        TEE->>TEE: Verify client attestation
        TEE->>TEE: Decrypt with 🔑 TEE Session Key<br/>📄 plaintext = decrypt(🔐blob)
        TEE->>TEE: Load into memory<br/>(Qdrant Neo4j in TEE)
        
        TEE-->>PyBackend: ✅ Data loaded
        
        PyBackend->>LocalDBs: Archive to encrypted_blobs table<br/>🔐 cold storage
        PyBackend->>LocalDBs: Clear hot indexes (free memory)
    end
    
    rect rgb(245, 245, 245)
        Note over User,TEE: Search via TEE
        
        User->>PyBackend: Search query
        PyBackend->>PyBackend: Check team data location
        
        alt Data in TEE
            PyBackend->>TEE: POST /search<br/>📊 {query, team_id}
            Note over TEE: Mutual TLS + attestation
            
            TEE->>TEE: Hybrid search in TEE memory<br/>(Qdrant + Neo4j)
            TEE-->>PyBackend: 📄 Search results
            
            PyBackend->>PyBackend: RAG generation (local LLM)
        else Data local
            PyBackend->>LocalDBs: Search locally (as normal)
        end
        
        PyBackend-->>User: Results
    end
    
    rect rgb(243, 245, 243)
        Note over PyBackend,LocalDBs: Load Back to Local
        
        User->>PyBackend: Disable TEE / work locally
        PyBackend->>TEE: POST /tee/export<br/>📊 {team_id}
        
        TEE->>TEE: Encrypt data<br/>🔐 export_blob = encrypt(🔑TEE Session Key, indexes)
        TEE-->>PyBackend: 🔐 encrypted export
        
        PyBackend->>PyBackend: Decrypt with 🔑 TEE Session Key
        PyBackend->>LocalDBs: Restore to Qdrant Neo4j
        
        PyBackend->>GoAPI: POST /tee/shutdown<br/>📊 {instance_id}
        GoAPI->>UserCloud: Terminate TEE instance
    end
    
    Note over TEE: 🟡 Hardware-protected enclave<br/>🔐 Data encrypted in transit to/from TEE<br/>🔑 Session keys derived from attestation<br/>🚫 Cloud provider cannot inspect TEE memory
    Note over GoAPI,Supabase: 🔴 Orchestration only<br/>📊 Metadata billing<br/>❌ NO access to plaintext data
```

**Security Properties:**
- 🟡 TEE provides hardware-level confidentiality (encrypted memory)
- 🔑 Session keys derived from TEE attestation + team key
- 🔐 Data encrypted during transit to/from TEE
- 🔒 Mutual TLS + attestation for all TEE communication
- 🚫 **Zero-Knowledge Boundary:** Cloud provider infrastructure cannot access TEE memory
- 🏢 User's cloud account hosts TEE (direct cost control)

**TEE Guarantees:**
- ✅ Hardware-protected confidential computing
- ✅ Attestation proves genuine secure enclave
- ✅ Cloud provider (AWS/Azure/GCP) cannot inspect TEE memory
- ✅ Encrypted communication channels only
- ✅ Data processed in hardware-isolated environment

**Workflow:**
1. **Provision:** Create TEE in user's cloud account
2. **Offload:** Encrypt and transfer data to TEE
3. **Process:** Search/compute runs in TEE enclave
4. **Load:** Transfer data back and shutdown TEE

**When TEE Required:**
- ⚠️ 50GB: Warning threshold
- 📊 100GB: Strong recommendation
- 🚫 500GB: Enforced requirement (local hardware insufficient)

---

## Summary: Zero-Knowledge Properties Across All Flows

| Flow | Trusted Zone | Untrusted Zone | What Cloud Sees | What Cloud CANNOT See |
|------|--------------|----------------|-----------------|----------------------|
| **Standalone Mode** | 🟢 User's device (all processing) | ❌ N/A (fully offline) | ❌ Nothing | ❌ N/A |
| **App-Managed Upload** | 🟢 Local encryption | 🔴 S3, Go API, Supabase | 🔐 Encrypted blob, 🔐 encrypted filename, 📊 metadata | 📄 Plaintext content, 📄 plaintext filename, 🔑 team key |
| **BYOS Upload** | 🟢 Local encryption | 🔴 User's bucket, Go API | 🔐 Encrypted blob, 🔐 encrypted filename, 🔐 encrypted credentials | 📄 Plaintext content, 🔑 credentials, 🔑 team key |
| **Search Chat** | 🟢 All search LLM processing | 🔴 Optional analytics | 📊 Query counts, timestamps | 📄 Query text, 📄 results, 📄 document content |
| **Download** | 🟢 Client-side decryption | 🔴 S3 serves ciphertext | 🔐 Encrypted blob download | 📄 Plaintext content, 🔑 decryption keys |
| **Share Links** | 🟢 Encryption decryption | 🔴 Share worker, metadata | 📊 Access logs, expiry, password hash | 🔑 Share key (in URL fragment), 📄 plaintext content |
| **Team Keys** | 🟢 Key generation unwrapping | 🔴 Supabase | 🔐 Encrypted team keys (per-user wrapped) | 🔑 Team key plaintext, 🔑 user master keys |
| **TEE Processing** | 🟢 Local backend, 🟡 TEE enclave | 🔴 Cloud infrastructure | 📊 TEE metadata, billing | 📄 Data in TEE memory (hardware-protected) |

---

## Key Takeaways

1. **🔑 Keys Never Leave Trusted Zones**
   - User master keys generated stored locally only
   - Team keys encrypted per-user before cloud storage
   - Share keys transmitted in URL fragments (client-only)

2. **🔐 Encryption Before Transmission**
   - All files encrypted with team keys before upload
   - Filenames encrypted and hex-encoded
   - BYOS credentials encrypted with org keys

3. **🚫 Zero-Knowledge Cloud**
   - Cloud sees only: encrypted blobs, encrypted filenames, metadata
   - Cloud cannot decrypt without keys (which it never receives)
   - Search and RAG processing stays local

4. **🟡 TEE Semi-Trusted Model**
   - Hardware-protected enclaves for large datasets
   - Attestation-based trust model
   - Cloud infrastructure cannot inspect TEE memory
   - User controls cloud account costs

5. **📊 Minimal Metadata Exposure**
   - Cloud receives only necessary operational metadata
   - Anonymized analytics (no query content)
   - Audit logs respect privacy vs compliance mode

6. **🔒 Defense in Depth**
   - Multiple encryption layers (transport + storage)
   - Mutual TLS + attestation for TEE
   - Time-limited presigned URLs
   - Permission checks before key distribution
